...
- `asr.py` — загрузка GigaAM и нарезка длинного аудио;
- `pyannote/__init__.py` — пустой совместимостный модуль: GigaAM проверяет его даже
  когда long-form API не используется;
...
- приложение пока не подписано сертификатом Apple Developer ID и не
  нотарифицировано, поэтому первый запуск выполняется через **Открыть** в
  контекстном меню.
